...
void VR::ProcessInput()
{
    ...
    auto sendOnTransition = [&](vr::VRActionHandle_t& action, const char* cmdOn, const char* cmdOff) {
        vr::InputDigitalActionData_t d;
        if (m_Input->GetDigitalActionData(action, &d, sizeof(d), vr::k_ulInvalidInputValueHandle) == vr::VRInputError_None && d.bChanged)
            m_Game->ClientCmd_Unrestricted(d.bState ? cmdOn : cmdOff);
    };
    sendOnTransition(m_ActionPrimaryAttack,   "+attack",  "-attack");
    sendOnTransition(m_ActionSecondaryAttack, "+attack2", "-attack2");
    sendOnTransition(m_ActionJump,            "+jump",    "-jump");
    sendOnTransition(m_ActionCrouch,          "+duck",    "-duck");
    sendOnTransition(m_ActionUse,             "+use",     "-use");
    sendOnTransition(m_ActionReload,          "+reload",  "-reload");
...
Vector VR::GetLeftControllerAbsPos(Vector eyePosition)
{
    Vector offset = eyePosition;
    if (offset.x == 0 && offset.y == 0 && offset.z == 0)
        offset = m_SetupOrigin;

    Vector position = offset + m_LeftControllerPosRel;
...
void VR::UpdateTracking()
{
    ...
    float rotCos = cos(rotY * 3.14159265f / 180.0f);
    auto pivotXY = [&](Vector& pt) {
        float x = pt.x * rotCos - pt.y * rotSin;
        float y = pt.x * rotSin + pt.y * rotCos;
        pt.x = x; pt.y = y;
    };
    pivotXY(hmdPosCorrected);
    ...
    // Apply rotation offset to left controller angles too
    ...
    static const PositionAngle viewmodelOffset = { {4.5f, -1.0f, 1.5f}, {0,0,0} };
